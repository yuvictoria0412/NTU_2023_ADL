...
def main():
    ...
    def preprocess_function(examples):
        ...
        second_sentences = [
            [context_json[num_paragraph] for num_paragraph in rows] for rows in examples["paragraphs"]
        ]
        ...
        tokenized_inputs['label'] = [
            paragraph.index(r_to_p_index)
            for paragraph, r_to_p_index in zip(examples['paragraphs'], examples['relevant'])
        ]
    ...
    metric_for_train = evaluate.load("accuracy")
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        accelerator.print(f"\n[validation data] epoch {epoch}: {eval_metric}")
        # logger.INFO(f"\n[validation data] epoch {epoch}: {eval_metric}")
        ...
        #     predictions = outputs.logits.argmax(dim=-1)
        #     predictions, references = accelerator.gather_for_metrics((predictions, batch["labels"]))
        #     metric_for_train.add_batch(
        #         predictions=predictions,
        #         references=references,
        #     )

        # eval_metric_2 = metric_for_train.compute()
        # accelerator.print(f"\n[train data] epoch {epoch}: {eval_metric_2}")
