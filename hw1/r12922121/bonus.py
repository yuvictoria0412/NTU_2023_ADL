...
def main():
    ...
    if args.dataset_name is not None:
        ...
        ''' where i don don so jiao '''
        def convert_context(example):
            # preprocess data
            whole_para = ""
            new_start = 0
            isInclude = True
            for x in example["paragraphs"]:
                whole_para += context_file[x]
                if x != example["relevant"] and isInclude:
                    new_start += len(context_file[x])
                else:
                    new_start += example["answer"]["start"]
                    isInclude = False
            
            example["context"] = whole_para
            example["answer"]["answer_start"] = [new_start]
            example["answer"]["text"] = [example["answer"]["text"]]
            example["answer"].pop("start")
            return example
        
        raw_datasets = raw_datasets.map(convert_context)
        raw_datasets = raw_datasets.rename_column("answer", "answers")
        ''' where i end don don so jiao '''
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                outputs = model(**batch)
                loss = outputs.loss
                step_loss.append(loss.cpu().item()/len(active_dataloader))
                # print(step_loss)
    ...
    if args.output_dir is not None:
        ...
        if accelerator.is_main_process:
            ...
            if args.with_tracking:
                ...
                logger.info(json.dumps(step_loss, indent=4))
